...
if args.swag:
    columns = columns[:-2] + ['swa_te_loss', 'swa_te_acc', 'sgd_ens_nll', 'sgd_ens_acc'] + columns[-2:]
    swag_res = {'loss': None, 'accuracy': None, 'sgd_ens_nll': None, 'sgd_ens_acc': None}
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.swag and (epoch + 1) > args.swag_start and (epoch + 1 - args.swag_start) % args.swag_c_epochs == 0:
        ...
        if epoch == 0 or epoch % args.eval_freq == args.eval_freq - 1 or epoch == args.epochs - 1:
            ...
            swag_res['sgd_ens_nll'] = sgd_ens_nll
            swag_res['sgd_ens_acc'] = sgd_ens_acc
        else:
            swag_res = {'loss': None, 'accuracy': None, 'sgd_ens_nll': None, 'sgd_ens_acc': None}
    ...
    if args.swag:
        values = values[:-2] + [swag_res['loss'], swag_res['accuracy'], swag_res['sgd_ens_nll'], swag_res['sgd_ens_acc']] + values[-2:]
